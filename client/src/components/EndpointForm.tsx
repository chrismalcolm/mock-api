...
type Props = {
  ...
  submitLabel: string,
  submitAction: (e: React.FormEvent, groupID: string, formData: IEndpoint | any) => void 
}

const EndpointForm: React.FC<Props> = ({ endpointid, groupid, initialPath, submitLabel, initialHttpMethod, initialResponseCode, initialRequestBody, initialResponseBody, submitAction }) => {  
  ...
  const [formData, setFormData] = useState<IEndpoint | {}>({
    _id: endpointid,
    path: initialPath,
  ...
  const handleFormInput = (e: React.FormEvent<HTMLInputElement>): void => {
    const field = e.currentTarget.id.substring(0, e.currentTarget.id.indexOf("|"));
    setFormData({
      ...formData,
  ...
  const handleTextAreaFormFactory = (id: string): ((e: React.ChangeEvent<HTMLTextAreaElement>) => void) => {
  ...
  return (    
      <form id={id} className='Form' onSubmit={(e) => submitAction(e, groupid, formData)}>
        <div>
          <div>
            <label htmlFor={pathID}>Path</label>
            <input onChange={handleFormInput} type='text' id={pathID} defaultValue={initialPath} autoComplete="off"/>     
            ...
            <select onChange={handleSelectForm} id={responseCodeID}>
              {ResponseCodes.map((ResponseCode: string) => {
                ...
                if (initialResponseCode === code) {
                  return (
                    <option value={code} selected>{ResponseCode}</option>
                  )
                }
                return (
                  <option value={code}>{ResponseCode}</option>
                )
              })}
            </select>
            <div hidden={hideRequestBodyTextArea()}>
              <label htmlFor={requestBodyID}>Request Body | <span className='Card--tab'
                onClick={() => {prettyTextArea(requestBodyID)}}
              ><b>Pretty</b></span><span> | </span><span className='Card--tab' 
                onClick={() => {compactTextArea(requestBodyID)}}
              ><b>Compact</b></span>
              </label>
              <textarea 
                id={requestBodyID}
            ...
            <div>
              <label htmlFor={responseBodyID}>Response Body | <span className='Card--tab' 
                onClick={() => {prettyTextArea(responseBodyID)}}
              ><b>Pretty</b></span><span> | </span><span className='Card--tab' 
                onClick={() => {compactTextArea(responseBodyID)}}
              ><b>Compact</b></span>
              </label>
              <textarea 
                id={responseBodyID}
        ...
        <div className="Card--button--container">
          ...
          <button
            className='Card--button__yellow'
            disabled={equalsCurrent()}
          >
            {submitLabel}
          </button>
        </div>
