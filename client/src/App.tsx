...
import { notifyWarn, notifyError, notifySuccess } from './components/Notifications'
...
import { getAllEndpoints, addEndpoint, updateEndpoint, deleteEndpoint } from './API'
...
const handleSaveGroup = (e: React.FormEvent, formData: IGroup): void => {
  ...
  const handleUpdateGroupFactory = (groupid: string): ((e: React.FormEvent, formData: IGroup | any) => void) => {
  ...
  const handleSaveEndpoint = (e: React.FormEvent, groupID: string, formData: IEndpoint): void => {
    e.preventDefault()
    ...
    .then(({ status, data }) => {
      if (status === 201) {
        notifySuccess(data.message);
        setEndpoints(data.endpoints)
        //let id = `formID|${groupID}:${formData._id}`
        //const form = document.getElementById(id) as typeof(EndpointForm);
        //form.resetInitial()
      } else {
        notifyWarn(data.errorMessage);
      };
  ...
  const handleUpdateEndpointFactory = (endpointid: string): ((e: React.FormEvent, groupID: string, formData: IEndpoint | any) => void) => {
  ...
  const renderEndpoints = (group: IGroup) => {
    return (
      <main className='App'>
        ...
        <h3 onClick={() => {window.location.href="/"}}>Back to Groups</h3>
        <EndpointAdd
          group={group} 
          saveEndpoint={handleSaveEndpoint}
        />
        {endpoints.filter(endpoint => endpoint.groupID === group._id).map((endpoint: IEndpoint) => (
          <EndpointItem
            key={endpoint._id}
