...
const addEndpoint = async (req: Request, res: Response): Promise<void> => {
    try {
        const groupid = req.params.groupid;
        const body = req.body as Pick<IEndpoint, "path" | "httpMethod" | "responseCode" | "requestBody" | "responseBody">;
...
const checkEndpointConflicts = (endpoint: IEndpoint, endpoints: IEndpoint[]): string => {
    var errMsg = "";
...
export { getEndpoints, getAllEndpoints, addEndpoint, updateEndpoint, deleteEndpoint };